...
class ProjectRepository:
    ...
    async def list(self, page: int, page_size: int, search: str | None, sort: str):
        ...
        total = await self.db.scalar(select(func.count()).select_from(Project).where(Project.deleted_at.is_(None)))
        return items, int(total or 0)
    async def get(self, project_id: UUID):
        return await self.db.scalar(select(Project).where(Project.id == project_id, Project.deleted_at.is_(None)))
    async def add(self, project: Project):
        self.db.add(project); await self.db.commit(); await self.db.refresh(project); return project
